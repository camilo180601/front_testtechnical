...
export default function EditarHotelPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        const getHotel = async () => {
            try {
                setLoading(true)
                const data = await fetchHotel(Number.parseInt(params.id))
                setHotel(data)
                setError(null)
            } catch (err) {
    ...
    const handleSubmit = async (data: Omit<Hotel, "id">) => {
        try {
            ...
            setError(null)
            await updateHotel(Number.parseInt(params.id), data)
            router.push(`/hoteles/${params.id}`)
        } catch (err) {
            console.error(err)
    ...
    if (loading) {
        return (
            <div className="container mx-auto py-8 px-4">
                <div className="mb-6">
                    <Link href={`/hoteles/${params.id}`} className="flex items-center text-blue-600 hover:text-blue-800">
